...
class MemberJoinRequestTest {
    ...
    void validation_Success() {
        // given
        MemberJoinRequestDto request = new MemberJoinRequestDto("testUser", "password123", "테스터");

        // when
        Set<ConstraintViolation<MemberJoinRequestDto>> violations = validator.validate(request);
    ...
    void validation_Fail_PasswordSize() {
        // given (비밀번호를 3자로 설정)
        MemberJoinRequestDto request = new MemberJoinRequestDto("testUser", "123", "테스터");
    ...
    void validation_Fail_NotBlank() {
        // given (아이디를 빈 값으로 설정)
        MemberJoinRequestDto request = new MemberJoinRequestDto("", "password123", "테스터");
